...
mask = path.join(folder,'mask_salonga_wwf.tif')
...
for input_file in input_files:
    ...
    for band_index in range(1, band_count):  # GDAL bands are 1-based
        ...
        output_file = path.join(
            folder, f"reclassified_{path.basename(input_file).split('.')[0]}_band{band_index}.tif"
        )
        ...
        gdal_calc.Calc(
            ...
            A_band= band_index,
            B=mask,
            calc='(1*((A<=20)&(A>=10))+2*((A>=51)&(A<=92))+3*((A>=120)&(A<=122))+4*(A==130)+5*((A>=181)&(A<=187))+6*(A==190)+7*((A>=200)&(A<=202))+7*((A>=150)&(A<=153))+8*(A==210))*B',
            outfile=output_file,
            NoDataValue=0,
